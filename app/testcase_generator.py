...
def generate_test_cases(metadata: dict, num_cases: int = 5) -> list:
    """
    Generate Codeforces-quality hidden test cases that catch subtle bugs.
    """
...
1. **ABSOLUTE EDGE CASES** (always include 2-3):
   - Minimum constraints (n=1, empty arrays if valid, single element)
   - Maximum constraints (n=10^5, values=10^9, stress memory/time)
...
2. **ALGORITHMIC TRAPS** (critical - include 1-2):
...
5. **PROBLEM-SPECIFIC TRAPS** (analyze description):
   ...
   - Create 1-2 cases specifically targeting that mistake
...
✓ At least 2 absolute edge cases (min/max values)
✓ At least 1 algorithmic trap (overflow, off-by-one, TLE)
✓ At least 1 problem-specific killer case
...
Now generate {num_cases} merciless test case inputs:
    ...
    # Parse response - handle multi-line test cases properly
    raw_content = response.content.strip()
    
    # Split by double newlines first (for multi-line test cases)
    test_cases = []
    current_case = []
